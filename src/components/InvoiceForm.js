...
handleCalculateTotal = () => {
    const subTotal = this.state.items.reduce((total, item) => {
      ...
      return total + itemTotal;
        ...
        // Parse GST rates from state
          const CGSTRate = parseFloat(this.state.CGST) || 0;
          const SGSTRate = parseFloat(this.state.SGST) || 0;
          const IGSTRate = parseFloat(this.state.IGST) || 0;
          
          // Calculate total GST number as a percentage
          const totalGSTNumber = CGSTRate + SGSTRate + IGSTRate;
          
          // Ensure givenAmount is a number
          const givenAmount = parseFloat(subTotal) || 0;

          // Calculate percentage of GST
          const percentageOfGST = totalGSTNumber > 0 
            ? (givenAmount / (1 + (totalGSTNumber / 100))).toFixed(2) 
            : '0.00';
          this.setState({
      ...
      this.setState({
        valueWithoutGst: parseFloat(percentageOfGST).toFixed(2),
        CGSTAmount: parseFloat(parseFloat(percentageOfGST) * (this.state.CGST / 100)).toFixed(2)
      }, () => {
        this.setState({
          SGSTAmount: parseFloat(parseFloat(percentageOfGST) * (this.state.SGST / 100)).toFixed(2)
        }, () => {
          this.setState({
              IGSTAmount: parseFloat(parseFloat(percentageOfGST) * (this.state.IGST / 100)).toFixed(2)
            }, () => {
              this.setState({
  ...
  render() {
const totalGST = (
  (parseFloat(this.state.CGST) || 0) +
  (parseFloat(this.state.SGST) || 0) +
  (parseFloat(this.state.IGST) || 0)
).toFixed(2);

// Convert totalGST back to a number for calculation
const totalGSTNumber = parseFloat(totalGST);

// Ensure givenAmount is a number
const givenAmount = parseFloat(this.state.subTotal) || 0;

// Calculate the percentage
const valueWithoutGst = totalGSTNumber > 0 ? (givenAmount / (1 + (totalGSTNumber / 100))).toFixed(2) : '0.00';
      ...
      <Row>
              ...
              <Col lg={6}>
                ...
                <div className="d-flex flex-row align-items-start justify-content-between" style={{
                  ...
                  <span className="fw-bold">{this.state.currency}
                    {(this.state.subTotal - this.state.discount) || 0}</span>
                </div>
              </Col>
            </Row>
            <hr className="my-4"/>
        ...
        <Col md={4} lg={3}>
          <div className="sticky-top pt-md-3 pt-xl-4">
            ...
            <InvoiceModal showModal={this.state.isOpen} closeModal={this.closeModal} info={this.state} invoiceNumber={this.state.invoiceNumber} valueWithoutGst={this.state.valueWithoutGst} items={this.state.items} currency={this.state.currency} subTotal={this.state.subTotal} discount={this.state.discount} NonTaxableAmount={this.state.NonTaxableAmount} CGSTAmount={this.state.CGSTAmount} SGSTAmount={this.state.SGSTAmount} IGSTAmount={this.state.IGSTAmount} total={this.state.total}/>
